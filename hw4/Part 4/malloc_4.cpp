...
void histogramRemove(MallocMetadata *data)
{
    ...
    // Check if we have a prev node in the histogram.
    if (data->hist_prev != nullptr)
    {
        // We have a prev entry. Skip the current entry.
    ...
    {
        // We do. Set it's prev to the current's prev.
        data->hist_next->hist_prev = data->hist_prev;
    }
    // Remove from the bucket by removing next and prev from node.
...
void uniteBlock(MallocMetadata *block)
{
    ...
    {
        // Merge the blocks into prev.
        // Remove the blocks from the histogram
        histogramRemove(block);
        histogramRemove(prev);
        // Update the size of the prev block.
        prev->size += block->size + sizeof(MallocMetadata);
        // Update the next block.
        prev->next = block->next;
        // Update the other pointer to correctly point at prev.
        if (block->next != nullptr)
        {
            block->next->prev = prev;
        }
        // Re-add the prev block to the histogram.
        histogramInsert(prev);
    }
}
...
void splitBlock(MallocMetadata *block, size_t new_size)
{
    ...
    block->size = new_size;
    // Get the new block from the secondary data in the prev block.
...
void *smalloc(size_t size)
{
    ...
    MallocMetadata *current, *prev;
    for (int i = histogramIndex(size); i < HIST_SIZE; i++)
    {
        current = prev = histogram[i];
        while (current != nullptr)
        {
...
void *srealloc(void *oldp, size_t size)
{
    ...
    if (current->size < size)
    {
        ...
        MallocMetadata *prev = current->prev, *next = current->next, *new_block;
        bool overwrite = false;
        if (prev != nullptr && prev->is_free && prev->size + current->size >= size)
        {
            // Appropriate to merge with prev.
            // Remove prev and current from histogram.
            histogramRemove(prev);
            histogramRemove(current);
            overwrite = size > prev->size;
            // We need to merge. Set the new size.
            prev->size += current->size + sizeof(MallocMetadata);
            // We need to update the pointers.
            // prev's pointers, next goes to current's next.
            prev->next = next;
            // Set the prev of next (is relevant)
            if (next != nullptr)
            {
                next->prev = prev;
            }
        ...
        else if (next != nullptr && next->is_free && next->size + current->size >= size)
        {
            ...
            new_block->is_free = false;
                 ...
                 prev->size + current->size + next->size >= size)
        {
            // Merge with prev and next.
            // Remove prev and current and next from histogram.
            histogramRemove(prev);
            histogramRemove(current);
            histogramRemove(next);
            // We need to merge. Set the new size.
            ...
            overwrite = size > prev->size;
            prev->size += current->size + next->size + 2 * sizeof(MallocMetadata);
            // We need to update the pointers.
            // prev's pointers, next goes to next's next.
            prev->next = next->next;
            // Set the prev of next's next (is relevant)
            if (next->next != nullptr)
            {
                next->next->prev = prev;
            }
        ...
        else if (current->next == nullptr)
        {
            ...
            if (current->prev != nullptr && current->prev->is_free)
            {
                ...
                // Remove the prev chunk from the histogram.
                // Extend the size inside the prev chunk.
                prev->size = size;
                // Set as used.
                prev->is_free = false;
                new_block = prev;
